...
class Flipkart2021(data.Dataset):
	def __init__(self, root, train_csvfile, vertical_attributes_npy, allowed_values_npy):
	    self.root = root
	    self.df = pd.read_csv(train_csvfile)
	    self.category_index_dict, self.category_attribute_map = self.get_category_index_dict(vertical_attributes_npy)
	    self.allowed_values = np.load(allowed_values_npy, allow_pickle=True).item()
	...
	def get(self, item):
	    ...
	    return img, category_label, attribute_value_label
